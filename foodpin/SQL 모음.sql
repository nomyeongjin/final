...
CREATE TABLE "REPORT" (
	...
	"REPORTER_NAME" NVARCHAR2(15) NOT NULL,
	"REVIEW_NO"	NUMBER		NOT NULL,
	"MEMBER_NO"	NUMBER		NOT NULL
);
...
-- 가게 정보 정정 처리 여부
ALTER TABLE "REQUEST_EDIT"  ADD
CONSTRAINT "REQUEST_EDIT_STATUS_CHECK"
CHECK("REQUEST_DEL_FL" IN('Y','N'));
...
SELECT TO_CHAR(REQUEST_DATE, 'YYYY-MM-DD (DY) PM HH:MI:SS')REQUEST_DATE 
FROM "REQUEST_EDIT";

SELECT * FROM "REQUEST_EDIT";
 ...
 INSERT INTO "REQUEST_EDIT" (
    ...
    SELECT STORE_IMG, STORE_NAME, STORE_NO, CATEGORY_TITLE
   FROM "STORE"
   JOIN "MEMBER" USING (MEMBER_NO)
   JOIN "CATEGORY" USING (STORE_NO)
...
SELECT * FROM "REVIEW"
JOIN "REVIEW_HASH" USING (REVIEW_NO)
JOIN "HASH" USING (HASH_NO)
WHERE MEMBER_NO=16
AND STORE_NO=8420348333
AND REVIEW_DEL_FL ='N';
...
SELECT STORE_IMG, STORE_NAME, STORE_NO, CATEGORY_CODE,CATEGORY_TITLE
   FROM "STORE"
   JOIN "MEMBER" USING (MEMBER_NO)
   JOIN "REVIEW_HASH" USING (REVIEW_NO)
  JOIN "HASH" USING (HASH_NO)
...
UPDATE "CATEGORY" SET CATEGORY_TITLE = '전체'
  ...
  WHERE STORE_NAME LIKE  '%아%';
     ...
     SELECT DISTINCT STORE_NO, STORE_NAME, STORE_INFO, STORE_LOCATION, STORE_STATUS,
       ...
       STORE_CLOSED, STORE_IMG,
	    ...
	    WHERE MEMBER_NO = 1
	    AND STORE_NAME LIKE '%아%') BOOKMARK
