...
/*가게 상세 조회 */
   SELECT S.STORE_NAME, S.STORE_INFO, S.STORE_LOCATION, S.STORE_STATUS,
       ...
       S.STORE_CLOSED, S.STORE_IMG, S.TOTAL_RATING, S.MEMBER_NO, 
       M.MENU_IMG_URL, M.MENU_NO, M.MENU_TITLE, M.MENU_AMOUNT 
		FROM "STORE" S
		LEFT JOIN STORE_CATEGORY SC ON S.STORE_NO = SC.STORE_NO
		LEFT JOIN WEEK_OFF W ON S.STORE_NO = W.STORE_NO
		LEFT JOIN DAY_OFF D ON S.STORE_NO = D.STORE_NO
		LEFT JOIN MENU M ON S.STORE_NO = M.STORE_NO
		LEFT JOIN REVIEW R ON S.STORE_NO = R.STORE_NO
		WHERE S.STORE_NO = 1;
